...
################################################################################################
p_est = np.zeros([imu_f.data.shape[0], 3])  # position estimates (10918,3)
v_est = np.zeros([imu_f.data.shape[0], 3])  # velocity estimates  (10918,3)
q_est = np.zeros([imu_f.data.shape[0], 4])  # orientation estimates as quaternions  (10918,4)
p_cov = np.zeros([imu_f.data.shape[0], 9, 9])  # covariance matrices at each timestep (10918,9,9)
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    #Kk:(9,3)
    Kk=p_cov_check@h_jac.T@np.linalg.inv(h_jac@p_cov_check@h_jac.T + R)
...
#    print(p_check.shape)
    ...
    p_check=p_check+delta_t*v_check+0.5*delta_t*delta_t*(Cns@imu_f.data[k-1]+g)
    ...
    f_jac[3:6,6:9]=-(Cns@skew_symmetric(imu_f.data[k-1].reshape(3,1)))*delta_t
    

    #print("f_jac: " + str(f_jac[3:6,6:9]))
   
    vfa=var_imu_f**2
    vfw=var_imu_w**2
    Q_km=delta_t*delta_t*np.diag([vfa,vfa,vfa,vfw,vfw,vfw])   #IMU Covariance 6x6
    #print(Q_km.shape)
    
    
    # 2. Propagate uncertainty
    p_cov_check=f_jac@p_cov_check@f_jac.T+l_jac@Q_km@l_jac.T
    
 
    
    # 3. Check availability of GNSS and LIDAR measurements
    for m in range (0, gnss.t.shape[0]):
        if(imu_f.t[k-1]==gnss.t[m]):
            y_k=gnss.data[m]
            gnss_i+=1
    ...
    for j in range (0, lidar.t.shape[0]):
        if(imu_f.t[k-1]==lidar.t[j]):
            y_k=lidar.data[j]
